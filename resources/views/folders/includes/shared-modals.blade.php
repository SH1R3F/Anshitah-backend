        ...
        <div class="modal-content">
            ...
            <form id="form-update-shared" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label>صلاحية المجلد</label>
                        <input id="update-shared-permission" type="text" name="permission" class="form-control"
                            required>
                    </div>
                </div>
                <div class="modal-footer">
    ...
    $(function(){
        ...
        $('.btn-update-shared').click(function(){
            ...
            $.get(`/shareds/details/${id}`,function(response){
                ...
                $('#update-shared-permission').val(response.name);
            });
        });
...
<script>
    $(function(){
        ...
        $('.btn-transfer').click(function(){
            var shared = $(this).data('shared');
            $.get(`/shared/files/${shared['id']}` , function(data){
                if(data.length > 0){
                    var html = '';
                    for(i = 0; i < data.length ; i++){
                        html += `<div class="mx-2">`;
                        html += `<label>${data[i]['file']}</label>`;
                        html += `<input type='checkbox' name="items[]" value='${data[i]['id']}'>`;
                        html += `</div>`;
                    }
                    $('#x-shared-files').html(html);
                }else{
                    $('#x-shared-files').html('<div class="alert alert-warning">المجلد لا بتوفر على ملفات</div>');
                }
            });
            $('#x-shared-file-name').html(shared['name']);
        });
    });
</script>
@endsection
